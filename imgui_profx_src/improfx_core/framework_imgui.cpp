...
namespace CoreModuleImGui {
    ...
    void RenderImGui::ImGuiInit(GLFWwindow* window_object, GuiConfig cfgdata) {
        ...
        if (!std::filesystem::exists(cfgdata.fonts_filepath))
            LogInfoPush(LOG_WARN, SYSTEM_MODULE_LABLE_imgui + "failed read fonts file.");
        else
            LogInfoPush(LOG_INFO, SYSTEM_MODULE_LABLE_imgui + "read fonts file: " + cfgdata.fonts_filepath);
    ...
    void RenderImGui::ImGuiFree() {
        ...
        LogInfoPush(LOG_INFO, SYSTEM_MODULE_LABLE_imgui + "free imgui context.");
    }
