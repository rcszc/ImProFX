...
namespace ImDynamicMem {
	...
	void ImMemory::OperMemoryPush(uint8_t* ptr, size_t bytes) {
		if (ptr) {
			...
			PushLogger(LOG_WARN, SYSMD_LB_MEMORY, "failed oper:push dy_memory, ptr = nullptr.");
	}
	...
	void ImMemory::OperMemoryResize(size_t bytes) {
		if (bytes != 0) {
			...
			PushLogger(LOG_WARN, SYSMD_LB_MEMORY, "failed oper:resize dy_memory, bytes = 0.");
	}
	...
	void ImMemory::OperMemoryErase(size_t offset, size_t bytes) {
		if (offset + bytes <= U8TdynamicMemory.size() && bytes != 0) {
			...
			PushLogger(LOG_WARN, SYSMD_LB_MEMORY, "failed oper:erase dy_memory, bytes = 0.");
		else
			PushLogger(LOG_WARN, SYSMD_LB_MEMORY, "failed oper:erase dy_memory, offset + bytes > size.");
	}
