...
bool TemplateCompReg(unordered_map<string, t_class*>& input, const char* key, t_class* comp, const char* name) {
	if (comp) {
		...
		if (!RepeatComp) {
			...
			const type_info& ObjectInfo = typeid(*comp);
			PushLogger(LOG_TRC, SYSMD_LB_ANIM, "register comp[%s]: %s", name, key);
			PushLogger(LOG_INFO, SYSMD_LB_ANIM, "register object: %s", ObjectInfo.name());
			return true;
		}
		else {
...
namespace ImGuiProAnim {
	...
	bool CallAnimButton(
		...
		if (BaseComp) {
			...
			{
				FlagTemp = ImGui::InvisibleButton(unique_name, ImVec2(ClassComp->ConfigNormalSize.vector_x, ClassComp->ConfigNormalSize.vector_y));
			...
			ImGui::Button(unique_name, ImVec2(BaseComp->AnimtransSize()->vector_x, BaseComp->AnimtransSize()->vector_y));
			...
			if (copy) {
				if (typeid(*copy) == typeid(ImAnimNodesHashmap)) {
					...
					*CompCreate->getnodes_refe() = *ObjectTemp->getnodes_refe();
					...
					PushLogger(LOG_WARN, SYSMD_LB_ANIM, "configure failed, copy object not'ImAnimNodesHashmap'.");
			}
			...
			if (copy) {
				if (typeid(*copy) == typeid(ImAnimNodesVector)) {
					...
					*CompCreate->getnodes_refe() = *ObjectTemp->getnodes_refe();
					...
					PushLogger(LOG_WARN, SYSMD_LB_ANIM, "configure failed, copy object not'ImAnimNodesVector'.");
			}
			ReturnBaseComp = CompCreate;
			break;
		}
