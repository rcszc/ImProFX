...
void GLEW_TEXTURE_USE2D(const uint32_t& number, FsTextureHandle& handle) {
	...
	glBindTexture(GL_TEXTURE_2D, handle.TextureHandle); // 绑定纹理到当前上下文.

	// texture => shader texture2d uniform.
	SFglobalUniform.uniform_integer(handle.ProgramHandle, handle.UniformName.c_str(), number);
}
...
void GLEW_TEXTURE_USE3D(const uint32_t& number, FsTextureArrayHandle& handle) {
	...
	glBindTexture(GL_TEXTURE_3D, handle.TextureHandle);
...
void GLEW_SHADER_DRAWMODEL(FsModelHandle& object_handle, const int& draw_vertexes) {
	// render => program.
	glBindVertexArray(object_handle.VaoHandle);
	glBindBuffer(GL_ARRAY_BUFFER, object_handle.VboHandle);
...
void FSdeleteHandle::delete_model_handle(FsModelHandle& modelobject) {
	PushLogger(LOG_INFO, SYSMD_LB_SHADER, "delete vertex_buffer object. ID: u%", modelobject.VboHandle);
	glDeleteBuffers(1, &modelobject.VboHandle);
}

void FSdeleteHandle::delete_texture_handle(FsTextureHandle& texture) {
	PushLogger(LOG_INFO, SYSMD_LB_SHADER, "delete texture object. ID: %u", texture.TextureHandle);
