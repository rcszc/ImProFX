...
#define	STCMODE_DELETE_ALL     0xAC03 // [all] delete object & erase item.
	...
	struct SrcImage {
		...
		uint8_t* ImagePixelPointer;
	};

	// consider async load config. 
	class FxShaderLoader {
	protected:
		FxRenderItem FxShaderPackage    = {};
		IFC_SHADER::ScriptLoadmode Mode = {};

		std::string ShaderVertString = {};
		std::string ShaderFragString = {};

		void ShaderFragmentScript(const std::string frag_script);

	public:
		FxShaderLoader(const std::string& script) {
			ShaderFragmentScript(script);
			Mode = IFC_SHADER::StringScript;
		}
		FxShaderLoader(const std::string& vert, const std::string& frag) {
			ShaderVertString = vert;
			ShaderFragString = frag;
			Mode = IFC_SHADER::StringFilePath;
		}
		uint32_t FxShaderProgramCreateCompile();

		// "vertexhandle"(opengl object handle): x:vbo, y:vao.
		...
		FxRenderItem* FxShader() { return &FxShaderPackage; }
	};

	class FrameworkCoreFind {
	public:
	...
	extern FrameworkCoreFind    FrmCore;     // 核心数据获取.
	extern FrameworkCoreManager FrmCoreOper; // 核心数据操作.
	extern FrameworkStaticFind  FrmStatic;   // "静态"数据操作.
	extern FrameworkTaskFind    FrmTask;     // "任务"操作.
}
...
namespace CoreModuleIMFX {
	...
	struct FRMCORE_PACKAGE {
		...
		ImThreadTask::ThreadingPool*                              TaskThreadPool; // thread pool.
