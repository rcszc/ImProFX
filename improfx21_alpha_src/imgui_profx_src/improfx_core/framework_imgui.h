...
#include "framework_imgui_control.hpp"
...
namespace ImGuiPro {
	...
	void FullScreenBackground(const uint32_t& TextureHandle, const bool& draw_fps = true);
	void FullWindowBackground(const uint32_t& TextureHandle, const ImVec2& uv_cropping = ImVec2(1.0f, 1.0f), const float& yoffset = 0.0f);
	...
	void FloatingWindowImage(const uint32_t& TextureHandle, const ImVec2& size = ImVec2(96.0f, 96.0f));
