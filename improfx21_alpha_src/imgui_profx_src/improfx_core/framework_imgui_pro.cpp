...
namespace basic_drawlist_comp {
	...
	void draw_image_uvscale(const uint32_t& TextureHandle, const ImVec2& position, const ImVec2& size, const float& scale) {
...
namespace ImGuiPro {
	...
	void FullScreenBackground(const uint32_t& TextureHandle, const bool& draw_fps) {
		ImGuiIO& io = ImGui::GetIO();

		ImGui::SetNextWindowPos(ImVec2(-IMGUI_ITEM_SPC * 0.5f, 0.0f));
		ImGui::SetNextWindowSize(ImVec2(io.DisplaySize.x + IMGUI_ITEM_SPC, io.DisplaySize.y));
		...
		ImGui::Image(CVT_IMPTR(TextureHandle), ImVec2(io.DisplaySize.x + IMGUI_ITEM_SPC * 0.5f, io.DisplaySize.y));
	...
	void FullWindowBackground(const uint32_t& TextureHandle, const ImVec2& uv_cropping, const float& yoffset) {
	...
	void TextTitleP2(const char* text, const float& child_winx, const ImVec4& color, const bool& border) {
		...
		basic_drawlist_comp::draw_text_scale(text, 1.8f, color);
		ImGui::EndChild();
	}
	...
	void RoundImage(const uint32_t& TextureHandle, const ImVec2& position, const ImVec2& size, const float& round, const ImVec2& uv_cropping) {
	...
	void FloatingWindowImage(const uint32_t& TextureHandle, const ImVec2& size) {
		ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.1f, 0.5f, 0.5f, 0.32f));
		ImGui::BeginTooltip();
		{
			ImGui::Image(CVT_IMPTR(TextureHandle), size);
		}
		ImGui::EndTooltip();
		ImGui::PopStyleColor();
	}
	...
	void CheckboxTriangle(const char* name, bool* p_flag) {
		...
		ImGui::ColorButton(name, color, ImGuiColorEditFlags_NoPicker | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoTooltip, size);
		if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0))
			*p_flag = !*p_flag;
		if (*p_flag)
			basic_drawlist_comp::draw_triangle_fill(
				ImVec2(position.x + 4.0f, position.y + 4.0f),
				ImVec2(16.0f, 0.0),
				ImVec2(8.0f, 16.0f),
...
#define SMOOTH_YSTEP 72.0f
	void SmoothYSlide(Vector2T<float>& ypos, const float& speed, const bool& wheel, const bool& focus) {
		// 聚焦当前窗口时生效.
