...
namespace ImGuiProAnimNodes {
	...
	*/
	...
	protected:
		std::unordered_map<std::string, IFC_ANIM::ImAnimDynamicNode> animation_nodes = {};

		// smooth transformation.
		Vector4T<float> anim_color[2]    = {};
		Vector2T<float> anim_size[2]     = {};
		Vector2T<float> anim_position[2] = {};
		// nodes flag.
		bool anim_flagstate = false;
	public:
		std::unordered_map<std::string, IFC_ANIM::ImAnimDynamicNode>* getnodes_refe() { return &animation_nodes; }

		bool UpdateTickName(const char* node_key, float smooth_scale) override;
		bool UpdateTickCount(size_t count, float smooth_scale) override { return false; }; // [EmptyIMP]

		void ComponentsInit() override {
			auto it = animation_nodes.begin();
			if (it != animation_nodes.end()) {

				anim_color[1]    = it->second.TransColor;
				anim_size[1]     = it->second.TransSize;
				anim_position[1] = it->second.TransPosition;
			}
			else
				LOGCONS::PushLogger(LOG_WARN, SYSMD_LB_ANIM, "nodes_anim init hashmap.size == 0.");
		};
		...
		Vector4T<float>* AnimtransColor() override { return &anim_color[1]; };
		Vector2T<float>* AnimtransSize() override { return &anim_size[1]; };
		Vector2T<float>* AnimtransPosition() override { return &anim_position[1]; };

		bool* AnimFlagState() { return &anim_flagstate; };
	...
	protected:
		// no unique key.
		std::vector<std::pair<std::string, IFC_ANIM::ImAnimDynamicNode>> animation_nodes = {};
		...
		bool anim_flagstate = false;
	public:
		std::vector<std::pair<std::string, IFC_ANIM::ImAnimDynamicNode>>* getnodes_refe() { return &animation_nodes; }

		bool UpdateTickName(const char* node_key, float smooth_scale) override { return false; }; // [EmptyIMP]
		bool UpdateTickCount(size_t count, float smooth_scale) override;

		void ComponentsInit() override {
			if (animation_nodes.size() > NULL) {

				anim_color[1]    = animation_nodes[0].second.TransColor;
				anim_size[1]     = animation_nodes[0].second.TransSize;
				anim_position[1] = animation_nodes[0].second.TransPosition;
			}
			else
				LOGCONS::PushLogger(LOG_WARN, SYSMD_LB_ANIM, "nodes_anim init vector.size == 0.");
		};
