...
namespace ImProFxTools {
	...
	public:
		...
		bool  StaticFlagDelete(std::unordered_map<std::string, bool>& input, strkey itemkey, const bool& erase = true);
		...
		bool StaticParticleSysDelete(std::unordered_map<std::string, IFC_PARTC::ParticleBase*>& input, strkey itemkey, const bool& ersae = true);
