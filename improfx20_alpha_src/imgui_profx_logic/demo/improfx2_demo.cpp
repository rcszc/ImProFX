...
bool ImprofxDemoClass::LogicInitialization(FRMCORE_PACKAGE& FrmDat) {

	SecWindowIcon.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "icon_settings"));
	SecWindowIcon.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "icon_arch"));
	SecWindowIcon.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "icon_info"));

	DemoMainPage.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_main"));
	DemoMainPage.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_arch"));
	DemoMainPage.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_core"));
	DemoMainPage.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_next"));

	DemoMainPageIcon.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_icon_shfx"));
	DemoMainPageIcon.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_icon_anim"));
	DemoMainPageIcon.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_icon_ptcs"));
	DemoMainPageIcon.push_back(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_icon_tool"));

	ArchImage = FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_arch_img");
...
void ImprofxDemoClass::SecondaryWindow(CoreModuleIMFX::FRMCORE_PACKAGE& FrmDat) {
	// 副窗口.
	...
	ImGui::BeginChild("AttachWindow", ImVec2(104.0f, 945.0f - IMGUI_ITEM_SPC * 2.0f));
	{
		...
		ImGui::Image(CVT_IMPTR(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "improfx2_logo")), ImVec2(88.0f, 88.0f));
		ImGui::Text("");

		ImGui::SetCursorPosX(IMGUI_ITEM_SPC - 2.0f);
		if (ImGuiProAnim::CallAnimButtonImage("img1", FrmDat.DataAnimation, "SecWinButA", FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "icon_settings"))) {
		...
		ImGui::SetCursorPosX(IMGUI_ITEM_SPC - 2.0f);
		if (ImGuiProAnim::CallAnimButtonImage("img2", FrmDat.DataAnimation, "SecWinButB", FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "icon_arch"))) {
		...
		ImGui::SetCursorPosX(IMGUI_ITEM_SPC - 2.0f);
		if (ImGuiProAnim::CallAnimButtonImage("img3", FrmDat.DataAnimation, "SecWinButC", FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "icon_info"))) {
		...
		ImGuiProAnim::CallAnimFixedWindow("Settings", FrmDat.DataAnimation, "WindowA", SecWindowFlags.vector_x,
			[&]() {
				...
				FrmData.FindSetRenderItemParam(FrmDat.DataShaderFx, "fx_background_shader", "brightness", SettingsBgColor.vector_y);
				FrmData.FindSetRenderItemParam(FrmDat.DataShaderFx, "fx_background_shader", "speed", SettingsBgSpeed.vector_y);
...
bool ImprofxDemoClass::LogicEventLoop(FRMCORE_PACKAGE& FrmDat, FRMCORE_INFO& FrmInfo) {
	...
	ImGuiPro::FullWindowBackground(FrmData.FindRenderItemFBO(FrmDat.DataShaderFx, "fx_background_shader"), ImVec2(0.85f, 1.0f), 8.0f);
	{
		ImGui::PopStyleColor();
		ImGui::PopStyleColor();
		...
		ImGuiProAnimNodes::CallAnimNodesVector(PageCount, FrmDat.DataAnimationNodes, "NodesNext", NODESFUNC{
				ImGui::Image(
					CVT_IMPTR(FrmData.FindRawTextureImg(FrmDat.DataTextureRaw, "demo2a_button_next")),
					ImVec2(size.vector_x, size.vector_y),
					ImVec2(0.0f, 0.0f),
					ImVec2(1.0f, 1.0f),
					ImVec4(color.vector_x, color.vector_y, color.vector_z, color.vector_w)
		...
		ImGui::BeginChild("NextPage1", ImVec2(1560.0f, 945.0f), true);
		{
			...
			ImGui::InputFloat2(u8"粒子生命随机分布", PTCcfgLife.data(), "%.2f");
			ImGui::SetNextItemWidth(372.0f);
			ImGui::InputFloat2(u8"粒子位置随机分布", PTCcfgPosition.data(), "%.2f");
			ImGui::SetNextItemWidth(372.0f);
			ImGui::InputFloat2(u8"粒子向量随机分布", PTCcfgVector.data(), "%.2f");

			ImGui::SetNextItemWidth(240.0f);
			ImGui::InputFloat(u8"粒子数量", &ParticleNumber);
			ImGui::SameLine();
			ImGuiPro::CheckboxTriangle("R", &PTCcfgColorSys.vector_x);
			ImGui::SameLine();
			ImGuiPro::CheckboxTriangle("G", &PTCcfgColorSys.vector_y);
			ImGui::SameLine();
			ImGuiPro::CheckboxTriangle("B", &PTCcfgColorSys.vector_z);
			...
			if (ImGuiProAnim::CallAnimButton(u8"生成粒子", FrmDat.DataAnimation, "PlayButPTC")) {
				...
				GenParticles.ConfigLifeDispersion(PTCcfgLife);
				GenParticles.ConfigRandomDispersion(PTCcfgVector, PTCcfgPosition, PTCcfgColorSys);
		...
		{
			ImGui::Image(CVT_IMPTR(FrmData.FindRenderItemFBO(FrmDat.DataShaderFx, "fx_circle_shader")), ImVec2(768.0f, 432.0f));
			ImGui::SameLine();
			ImGui::Image(CVT_IMPTR(FrmData.FindRenderItemFBO(FrmDat.DataShaderFx, "fx_hexerino_shader")), ImVec2(768.0f, 432.0f));
...
void ImprofxDemoClass::LogicCloseFree() {
