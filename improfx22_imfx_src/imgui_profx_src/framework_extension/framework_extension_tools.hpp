...
#define EXT_LOGGER_BUFFER_LEN 3072
	...
	inline void ELog(const EXTLOGLABEL& Label, const char* ModuleLabel, const char* LogText, ...) {
		...
		vsnprintf(LoggerStrTemp, EXT_LOGGER_BUFFER_LEN, LogText, ParamArgs);
		va_end(ParamArgs);
...
namespace EXTIFC {
	...
	// extension gui_control animation interface.
	class EXT_GUICONTROL_ANIM {
	public:
		virtual void UpdateFrameGetUniqueIDs(uint32_t* CUID) = 0;
		virtual void ResetInterSpeedScale(float scale) = 0;
	};
}
