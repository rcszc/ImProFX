	...
	*/
	...
	bool FrameworkCoreManager::FindLoadRawTextureImgData(unordered_map<string, string>& input, strkey itemkey, const SrcImage& data) {
		...
		if (it != input.end()) {
			...
			// source texture => mid_name:"SRCTEX"
			auto StorageName = string(itemkey) + LLRES_ADCHAR + "SRCTEX" + LLRES_ADCHAR;

			if (TextureProcess.CreateTexture(data.ImageWidth, data.ImageHeight, data.ImageChannels, data.ImagePixelPointer, LinearFiltering))
