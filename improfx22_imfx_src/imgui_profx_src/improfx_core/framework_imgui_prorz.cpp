        ...
        ImGuiWindowChild(name,
            [&]() {
                for (const auto& item : pageimgs) {
                    ...
                    Image(CVT_IMPTR(item), winsize, ImVec2(0.0f, 0.0f), ImVec2(1.0f, 1.0f), ImVec4(1.0f, 1.0f, 1.0f, transparent));
                    SameLine();
                }
            ...
            ImVec2(winsize.x + IMGUI_ITEM_SPC, winsize.y + IMGUI_ITEM_SPC * 2.0f), border, winflags
        );
    }
...
#define DEBUG_ITEM_WIDTH 208.0f
    void FrameworkGUI::RZsystemDebugWindow(DebugWindowBuffer& buffer, bool* p_open) {
        ...
        ImGuiWindow("> Framework20 ImGui Debug",
            [&]() {
