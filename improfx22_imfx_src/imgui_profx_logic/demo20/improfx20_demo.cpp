...
void ImprofxDemoClass20::SecondaryWindow(FRMCORE_PACKAGE& FrmDat, FRMCORE_INFO& FrmInfo) {
	// 副窗口.
...
bool ImprofxDemoClass20::LogicEventLoop(FRMCORE_PACKAGE& FrmDat, FRMCORE_INFO& FrmInfo) {
	...
	FrmInfo.ImProFXGUI->PRO_FullWindowBackground(FrmCore.FindRenderItemFBO(FrmDat.DataShaderFx, "fx_background_shader"), ImVec2(0.85f, 1.0f), 8.0f);
	{
		ImGui::PopStyleColor();
		ImGui::PopStyleColor();
		...
		ImGui::SetCursorPosX(FrmInfo.ImProFXGUI->PRO_ItemCalcCentered(768.0f + IMGUI_ITEM_SPC) + 50.0f + IMGUI_ITEM_SPC);
