...
namespace ShaderGlobalRes {
	...
	void ResourcesTMU::TmuCountFreeTex2D(FsTextureHandle& Handle) {
		...
		if (Handle.tmu_count >= TmuStateFlag.size())
			PushLogger(LOG_ERR, SYSMD_LB_SHADER, "global_res tmu texture2d_free count > tmu.");
		else {
			TmuStateFlag[(size_t)Handle.tmu_count] = false;
			PushLogger(LOG_TRC, SYSMD_LB_SHADER, "global_res tmu texture2d_free count: %u", Handle.tmu_count);
		}
	}
	...
	void ResourcesTMU::TmuCountFreeTex3D(FsTextureArrayHandle& Handle) {
		...
		if (Handle.tmu_count >= TmuStateFlag.size())
			...
			TmuStateFlag[(size_t)Handle.tmu_count] = false;
			PushLogger(LOG_TRC, SYSMD_LB_SHADER, "global_res tmu texture3d_free count: %u" + Handle.tmu_count);
		}
	}
