...
namespace ImProFxTools {
	...
	bool FrameworkStaticFind::StaticFlagDelete(unordered_map<string, bool>& input, strkey itemkey) {
		auto it = input.find(itemkey);
		if (it != input.end()) {
			input.erase(it);
	...
	bool FrameworkStaticFind::StaticTimerDelete(unordered_map<string, ChronoTimer*>& input, strkey itemkey) {
		auto it = input.find(itemkey);
		if (it != input.end()) {
			if (it->second)
				delete it->second;
	...
	bool FrameworkStaticFind::StaticParticleSysDelete(unordered_map<string, IFC_PARTC::ParticleBase*>& input, strkey itemkey) {
		auto it = input.find(itemkey);
		if (it != input.end()) {
			if (it->second)
				delete it->second;
