...
uniform float BloomFilter; // 辉光颜色过滤(RGB均值).
...
vec4 FilterColor(vec4 in_colors, float value)
{
    float sum_color = in_colors.x + in_colors.y + in_colors.z;
    float average_color = sum_color / 3;

    if (average_color > value) {
        return in_colors;
    } 
    else {
	...
	if (FxSwitch > 0) {
		...
		for (int i = -BlurRadius; i <= BlurRadius; i++) {
			for (int j = -BlurRadius; j <= BlurRadius; j++) {
				...
				BlurColor += FilterColor(texture(GuiTexture, FxCoord + BlurOffset), BloomFilter) * Kernel[i + BlurRadius] * Kernel[j + BlurRadius];
			}
		}

		vec4 SourceColor = texture(GuiTexture, FxCoord);
		FragColor = vec4(BlurColor.rgb * BloomBlur + SourceColor.rgb * BloomSource, 1.0f);
	}
	else {
