...
namespace IMFX_OGL_RES {
	...
	bool ImFXOGLresShader::ResourceStorage(ResUnique key, ImFxGLmanagerShader* res) {
		...
		if (it != ResourceShaderMap.end()) {
			...
			if (ResFlag == DEFRES_FLAG_NORMAL) {
				...
				ImFXLog(LOG_WARNING, IMFX_OGLRES_LABEL, "shader: failed storage, key: %s, code: %i", key.c_str(), ResFlag);
				return false;
			}
		}
	}
	...
	bool ImFXOGLresTexture::ResourceStorage(ResUnique key, ImFxGLmanagerTexture* res) {
		...
		if (it != ResourceTextureMap.end()) {
			...
			if (ResFlag == DEFRES_FLAG_NORMAL) {
				...
				ImFXLog(LOG_WARNING, IMFX_OGLRES_LABEL, "texture: failed storage, key: %s, code: %i", key.c_str(), ResFlag);
				return false;
			}
		}
	}
	...
	bool ImFXOGLresVertexBuffer::ResourceStorage(ResUnique key, ImFxGLmanagerModel* res) {
		...
		if (it != ResourceVertexBufferMap.end()) {
			...
			if (ResFlag == DEFRES_FLAG_NORMAL) {
				...
				ImFXLog(LOG_WARNING, IMFX_OGLRES_LABEL, "verbuffer: failed storage, key: %s, code: %i", key.c_str(), ResFlag);
				return false;
			}
		}
	}
	...
	bool ImFXOGLresFrameBuffer::ResourceStorage(ResUnique key, ImFxGLmanagerFrameBuffer* res) {
		...
		if (it != ResourceFrameBufferMap.end()) {
			...
			if (ResFlag == DEFRES_FLAG_NORMAL) {
				...
				ImFXLog(LOG_WARNING, IMFX_OGLRES_LABEL, "framebuf: failed storage, key: %s, code: %i", key.c_str(), ResFlag);
				return false;
			}
		}
	}
	...
	RenderBufferAttr ImFXOGLresRenderBuffer::ResourceFind(ResUnique key) {
		std::lock_guard<std::mutex> Lock(ResourceRenderBufferMutex);
		return (ResourceRenderBufferMap.find(key) != ResourceRenderBufferMap.end()) ? ResourceRenderBufferMap[key] : RenderBufferAttr();
	}

	bool ImFXOGLresRenderBuffer::ResourceStorage(ResUnique key, ImFxGLmanagerRenderBuffer* res) {
		std::lock_guard<std::mutex> Lock(ResourceRenderBufferMutex);

		auto it = ResourceRenderBufferMap.find(key);
		if (it != ResourceRenderBufferMap.end()) {
			ImFXLog(LOG_WARNING, IMFX_OGLRES_LABEL, "renderbuf: failed storage duplicate_key: %s", key.c_str());
			return false;
		}
		else {
			ResourceFlag ResFlag = {};
			...
			if (ResFlag == DEFRES_FLAG_NORMAL) {
				...
				ImFXLog(LOG_INFO, IMFX_OGLRES_LABEL, "renderbuf: storage key: %s", key.c_str());
				return true;
			}
			else {
				ImFXLog(LOG_WARNING, IMFX_OGLRES_LABEL, "renderbuf: failed storage, key: %s, code: %i", key.c_str(), ResFlag);
				return false;
			}
		}
	}

	bool ImFXOGLresRenderBuffer::ResourceDelete(ResUnique key) {
		std::lock_guard<std::mutex> Lock(ResourceRenderBufferMutex);
		...
		if (it != ResourceRenderBufferMap.end()) {
			...
			ImFXLog(LOG_INFO, IMFX_OGLRES_LABEL, "renderbuf: delete key: %s", key.c_str());
			return true;
		}
		else {
			ImFXLog(LOG_WARNING, IMFX_OGLRES_LABEL, "renderbuf: failed delete, not found key.");
			return false;
		}
	}
}