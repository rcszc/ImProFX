...
void ImprofxDemoClass22::ConfigAnimation(CoreModuleIMFX::FRMCORE_PACKAGE& FrmDat) {
	// -------------------------------- ImGuiProAnim --------------------------------
	...
	WindowArch->ConfigOpenSize = Vector2T<float>(280.0f, 350.0f);
	...
	auto TempSize = Vector2T<float>(1000.0f, 521.0f);

	AnimNodesArch->ConfigNodeAdd("T1", ImGuiProAnimNodes::ConfigAnimNode(true, Vector2T<float>(400.0f, -300.0f), TempSize, Vector4T<float>()));
	AnimNodesArch->ConfigNodeAdd("T2", ImGuiProAnimNodes::ConfigAnimNode(true, Vector2T<float>(400.0f, 300.0f), TempSize, Vector4T<float>(1.0f, 1.0f, 1.0f, 1.0f)));
	AnimNodesArch->ConfigNodeAdd("T3", ImGuiProAnimNodes::ConfigAnimNode(true, Vector2T<float>(400.0f, -300.0f), TempSize, Vector4T<float>()));
