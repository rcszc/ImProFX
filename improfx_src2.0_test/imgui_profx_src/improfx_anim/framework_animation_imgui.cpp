...
namespace ImGuiProAnim {
	...
	void AnimCompReg(unordered_map<string, ImAnimationBase*>& input, const char* key, ImAnimationBase* comp) {
		if (comp) {
			...
			LogInfoPush(LOG_TRC, SYSMD_LB_ANIM + "register comp: " + key);
			LogInfoPush(LOG_INFO, SYSMD_LB_ANIM + "register obj_rtti: " + objinfo.name());
		}
		else
			LogInfoPush(LOG_ERR, SYSMD_LB_ANIM + "failed register comp nullptr.");
	}

	void AnimCompDelete(std::unordered_map<std::string, ImAnimationBase*>& input, const char* key) {
		ImAnimationBase* comp = FindAnimComp(input, key);
		if (comp) {
			delete comp;
			LogInfoPush(LOG_INFO, SYSMD_LB_ANIM + "failed delete comp: " + key);
		}
		else
			LogInfoPush(LOG_WARN, SYSMD_LB_ANIM + "failed delete comp nullptr.");
	}
