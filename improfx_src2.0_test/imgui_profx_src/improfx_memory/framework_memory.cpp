...
namespace ImDynamicMem {
	...
	bool memory_operations(MemMode mode, MemType src_type, MemType type, const char* oper) {
		...
		if (mode == SingPatt) {
			if (src_type == type)
				...
				LogInfoPush(LOG_ERR, SYSMD_LB_MEMORY + "failed mode: 'SingPatt' type error, oper: " + oper);
		}
		else
			returnexeflag = true;
	...
	void DynamicMemCreateReg(unordered_map<string, ImMemory*> input, const char* key, MemType type, size_t length, MemMode mode) {
		ImMemory* memobj = new ImMemory(mode);

		memobj->AllocDynamicMem(type, length);
		input[key] = memobj;

		LogInfoPush(LOG_TRC, SYSMD_LB_MEMORY + "register memory: " + key);
	}
