...
#define SYSMD_LB_SHADER std::string("[ogl_shader]: ")
...
protected:
	...
	bool repeat_compile;
	typemod loadmod[2] = {}; // vs.mod, fs.mod.
...
public:
	...
	~FSloadShader() {
		// free shader data.
		if (source_vertex_shader != nullptr && loadmod[0] == StringFilePath) 
			delete[] source_vertex_shader;
		if (source_fragment_shader != nullptr && loadmod[1] == StringFilePath) 
	...
	// load_shader.
	void load_shader_vs(std::string VsFile, typemod Mod = StringFilePath);
	void load_shader_fs(std::string FsFile, typemod Mod = StringFilePath);
	/*
	* @param  void
...
public:
	...
	// load source data: param: x: width, y:height, z:color_ch. [safe-warning].
	void load_srcdata_texture(uint8_t* ptr, Vector3T<uint32_t> param);
};
...
protected:
	uint32_t global_vertexarray = NULL;
	uint32_t global_vao_begin = NULL;

	void _glew_vertexarray_object(uint32_t vertex_length_bytes, uint32_t begin);
	void _glew_vertexbuffer_object(bool createvao, uint32_t& VAO, uint32_t& VBO, uint32_t data_length_byte, float* vertex_data);

public:
	// create vbo, vao(?).
	// location end = begin + 4, [begin, begin + 4].
	void load_model_data(FSdynamic Input, FSmodelHanlde& ObjectHandle, const bool& CreateVao = true);

	// data location count begin.
	void set_vaoatt_begin(uint32_t Begin) { global_vao_begin = Begin; };
};
