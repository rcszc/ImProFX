...
namespace CoreModuleImGui {
    ...
    void RenderImGui::ImGuiInit(GLFWwindow* window_object, GuiConfig cfgdata) {
        ...
        if (!std::filesystem::exists(cfgdata.fonts_filepath))
            LogInfoPush(LOG_WARN, SYSMD_LB_IMGUI + "failed read fonts file.");
        else
            LogInfoPush(LOG_INFO, SYSMD_LB_IMGUI + "read fonts file: " + cfgdata.fonts_filepath);
    ...
    void RenderImGui::ImGuiFree() {
        ...
        LogInfoPush(LOG_INFO, SYSMD_LB_IMGUI + "free imgui context.");
    }
