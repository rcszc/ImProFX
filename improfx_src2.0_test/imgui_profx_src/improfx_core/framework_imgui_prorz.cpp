...
#define FDSA_CHILD_WIN_HIGH 36.0f
    void TemplatePageFDSA(
        ...
        float window_height = (float)temp_data.size() * (FDSA_CHILD_WIN_HIGH + IMGUI_ITEM_SPC) + IMGUI_ITEM_SPC;

        ImGui::BeginChild(name, ImVec2(640.0f, window_height), border);
        {
        ...
        ImGui::Begin("> Framework20 ImGui Debug", p_open, ImGuiWindowFlags_NoResize);
        {
            ...
            if (logwindow_flag) {
                ...
                for (const auto& logstr : LOGCONS::ReadLogCache::ExtractLogSegment(logwindow_lines)) {
                    ...
                    if (logstr.find(LOG_INFO) != string::npos) color = ImVec4(1.0f, 1.0f, 1.0f, 0.72f);
                    if (logstr.find(LOG_ERR) != string::npos)  color = ImVec4(1.0f, 0.0f, 0.0f, 0.72f);
                    if (logstr.find(LOG_WARN) != string::npos) color = ImVec4(1.0f, 1.0f, 0.0f, 0.72f);
                    if (logstr.find(LOG_TRC) != string::npos)  color = ImVec4(0.0f, 1.0f, 1.0f, 0.72f);
                    if (logstr.find(LOG_PERF) != string::npos) color = ImVec4(0.78f, 0.0f, 1.0f, 0.72f);

                    ImGui::TextColored(color, logstr.c_str());
                }
                ImGui::End();
            }
