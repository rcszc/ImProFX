...
namespace ImGuiPro {
	...
	void FullScreenBackground(uint32_t texture_hd, bool draw_fps) {
		...
		ImGui::SetCursorPos(ImVec2(IMGUI_ITEM_SPC, 0.0f));
		if (draw_fps)
	...
	void FullWindowBackground(uint32_t texture_hd) {
		ImGui::SetCursorPos(ImVec2(0.0f, 0.0f));
		ImGui::Image(CVT_IMPTR(texture_hd), ImVec2(ImGui::GetWindowSize().x, ImGui::GetWindowSize().y - IMGUI_ITEM_SPC));
	...
	void MouseFloatingWindow(ImVec2 size, bool focus, function<void()> window_ui) {
		...
		ImGui::SetNextWindowPos(ImVec2(io.MousePos.x + 32.0f, io.MousePos.y + 32.0f), ImGuiCond_Always);
