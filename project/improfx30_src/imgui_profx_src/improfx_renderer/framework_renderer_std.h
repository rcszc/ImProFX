...
/* 
* ImProFX IMFXGL 图形抽象层标准接口 Version: 0.1.8, Update: 2024.02.03
* 着色器支持: 顶点着色器, 片元着色器.
* 当前兼容图形库: OpenGL
...
public:
	...
	virtual ImageRawData ReadRenderBuffer(RenderBufferAttr buffer) = 0;

	virtual RenderBufferAttr _MS_GETRES(ResourceFlag& flag) = 0;
};

class ImFxGLmanagerUniform {
public:
...
// ******************************** Input/Output ********************************
// Renderer IMFXGL, IO_Manager.

class ImFxIOmanagerImage {
public:
	virtual bool WriteImageFile(ImageRawData rawdata, std::string file, SaveImageMode mode, float quality) = 0;
