...
namespace ImGuiPro {
	...
	void FullScreenImWindowBg(ImTextureID texture_hd, const bool& draw_fps) {
		...
		ImGui::Image(texture_hd, ImVec2(ImIO.DisplaySize.x + IMGUI_ITEM_SPC * 0.5f, ImIO.DisplaySize.y));
	...
	void FrameworkImGuiControl::PRO_FloatingWindowImage(ImTextureID texture_hd, const ImVec2& size) {
		ImGuiWindowTootip([&]() {
			Image(texture_hd, size);
		});
	}
	...
	void FrameworkImGuiControl::PRO_CheckboxTriangle(const char* name, bool* p_flag) {
		...
		ImVec2 DwSize = ImVec2(30.0f, 30.0f);
...
#define SMOOTH_YSTEP 72.0f
	void FrameworkImGuiControl::PRO_SmoothYSlide(Vector2T<float>& ypos, float speed, bool wheel, bool focus) {
		if (IsWindowFocused() && ImGui::IsWindowHovered() || !focus) {
			if (wheel) {
