...
namespace ImParticle {
	...
	void ImPTCgenerator::ConfigRandomDispersion(Vector2T<float> rand_limit_vector, Vector2T<float> rand_limit_position, Vector3T<bool> color_system, Vector3T<float> offset_position) {
		...
		ColorSystemType = color_system; OffsetPosition = offset_position;
	}
	...
	void ImPTCgenerator::CreateAddParticleDataset(vector<ParticleAttributes>& data) {
		...
		case(PTCpoints): {
			for (size_t i = 0; i < ParticlesNumber; ++i) {
				...
				particle_colorsys(ParticleTemp, ColorSystemType);
...
#define DEFAULT_SPEED 1.0f
	void ImParticleFx::CalcUpdateParticles(vector<ParticleAttributes>& particles, float speed, float lifesub) {
	...
	ImParticleFx::ImParticleFx(const Vector2T<float>& window_size) {
		...
		ShaderProcess.ShaderLoaderVS(FrameworkSystemPath[2]);
		ShaderProcess.ShaderLoaderFS(FrameworkSystemPath[3]);
		...
		// 粒子着色器单独创建'VAO'.
		if (LLRES_VertexAttributes->ResourceFind("VER_ATTR_PARTIC") == OGLinvalidHandle)
			LLRES_VertexAttributes->ResourceStorage("VER_ATTR_PARTIC", ModelProcess.CreateVertexAttribute(0, 0));
		
		if (ModelProcess.CreateDynamicModel(LLRES_VertexAttributes->ResourceFind("VER_ATTR_PARTIC"), VerBufferTemp, nullptr, NULL))
			LLRES_VertexBuffers->ResourceStorage(VertexModelItem, &ModelProcess);

		float RenderScale = window_size.vector_x / window_size.vector_y;
		...
		if (FboTexProcess.CreateTextureEmpty((uint32_t)window_size.vector_x, (uint32_t)window_size.vector_y, 4, LinearFiltering))
		...
		PushLogger(LOG_INFO, SYSMD_LB_PARTICLE, "particle system render_resolution: %.2f x %.2f", window_size.vector_x, window_size.vector_y);
	}
	...
	void ImParticleFx::_UPDATE_ParticleSystem() {
		...
		RenderingSupport::RnenderBindFrameBuffer(LLRES_FrameBuffers->ResourceFind(FrameBufferItem));
		{
