	...
	namespace Tools {
		...
		std::vector<std::vector<T>> Vector1DConvertTo2D(const std::vector<T>& input, size_t x, size_t y) {
			...
			if (input.size() != VectorLength) {
				...
				LOGCONS::PushLogger(LOG_ERROR, SYSMD_LB_MEMORY, "failed tool:'Vector1DConvertTo2D' 1D => 2D param.");
			return ResultMat;
		}
		...
		std::vector<T> BytesConvertToFloat(const std::vector<uint8_t>& input) {
			...
			if (InSize < sizeof(T)) {
				LOGCONS::PushLogger(LOG_ERROR, SYSMD_LB_MEMORY, "failed tool:'BytesConvertToFloat' size < sizeof(T).");
				return std::vector<T>();
			}
			...
			if (InSize % sizeof(T) == 0) {
				...
				LOGCONS::PushLogger(LOG_WARNING, SYSMD_LB_MEMORY, "failed tool:'BytesConvertToFloat' size % sizeof(T) != 0.");
				return std::vector<T>();
			}
		}
	}
